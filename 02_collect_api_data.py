...
# ============================================================
# R2 DATA COLLECTION  (Rev. 5 - token threshold & reasoning fix)
# ------------------------------------------------------------
# WHAT CHANGED vs Rev.4 (and WHY):
#   1) MAX TOKEN CAP OVERRIDE: Ollama's "num_predict" parameter is a hard stop.
#      To allow token counts to exceed MAX_TOKENS and capture the full output,
#      the API request is sent with a loose maximum ceiling (8192). The script
#      then flags "truncated": 1 mathematically in the CSV if the returned 
#      token_count >= MAX_TOKENS (2048), keeping the full text intact.
#   2) REASONING EFFORT FIX: Preserved the dynamic mapping of THINK = False to
#      "reasoning_effort": "minimal" for cloud models to avoid HTTP 400 errors.
# ============================================================


# ============================================================
# CONFIGURATION   (Tasin's setup preserved: 5 models, cloud tags, port 11434)
# ============================================================
...
RETRY_LIMIT         = 3
TIMEOUT             = 300    # reasoning models on cloud can be slow
...
def call_ollama(model_name, prompt, temperature=TEMPERATURE):
    ...
    # --- CLOUD PROVIDER ADAPTATION LAYER FOR REASONING EFFORT ---
    if not THINK:
...
# ============================================================
# MAIN EXECUTION (per model) - auto-resume + batch-save
# ============================================================
